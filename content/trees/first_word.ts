import type { ExerciseTree } from "../../lib/exercise/model";


export const firstWordTree: ExerciseTree = {
  "startNodeId": "fw_decision_1",
  "nodes": {
    "fw_decision_1": {
      "id": "fw_decision_1",
      "type": "question",
      "context": "نبدأ من الكلمة الأولى فقط.",
      "text": "ما نوع الكلمة المحددة في الجملة؟",
      "hint": "هل تدل الكلمة المحددة على اسم أو معنى بلا زمن، أم على حدث وزمن، أم لا يظهر معناها كاملًا إلا مع غيرها؟",
      "answers": [
        {
          "id": "a",
          "text": "اسم",
          "next": "R_first_noun",
          "eval": {
            "fact": "wordType",
            "equals": "noun"
          }
        },
        {
          "id": "b",
          "text": "فعل",
          "next": "fw_verb_tense",
          "eval": {
            "fact": "wordType",
            "equals": "verb"
          }
        },
        {
          "id": "c",
          "text": "حرف",
          "next": "fw_particle_after",
          "eval": {
            "fact": "wordType",
            "equals": "particle"
          }
        }
      ]
    },
    "fw_verb_tense": {
      "id": "fw_verb_tense",
      "type": "question",
      "context": "عرفنا أن الكلمة فعل.",
      "text": "ما نوع الفعل؟",
      "hint": "هل يدل الفعل على حدث وقع وانتهى، أم على حدث يقع أو يتجدد، أم على طلب حصول الحدث؟",
      "answers": [
        {
          "id": "a",
          "text": "ماضٍ",
          "next": "R_first_past",
          "eval": {
            "fact": "verbType",
            "equals": "past"
          }
        },
        {
          "id": "b",
          "text": "مضارع",
          "next": "R_first_present",
          "eval": {
            "fact": "verbType",
            "equals": "present"
          }
        },
        {
          "id": "c",
          "text": "أمر",
          "next": "R_first_imperative",
          "eval": {
            "fact": "verbType",
            "equals": "imperative"
          }
        }
      ]
    },
    "fw_particle_after": {
      "id": "fw_particle_after",
      "type": "question",
      "context": "عرفنا أن الكلمة حرف.",
      "text": "ما نوع الكلمة التي جاءت بعد الحرف؟",
      "hint": "انظر إلى الكلمة التي جاءت بعد الحرف: هل تدل على حدث وزمن فتكون فعلًا، أم تدل على معنى بلا زمن فتكون اسمًا؟",
      "answers": [
        {
          "id": "a",
          "text": "جاء بعده فعل",
          "next": "R_first_particle_verb",
          "eval": {
            "fact": "afterParticle",
            "equals": "verb"
          }
        },
        {
          "id": "b",
          "text": "جاء بعده اسم",
          "next": "R_first_particle_noun",
          "eval": {
            "fact": "afterParticle",
            "equals": "noun"
          }
        }
      ]
    },
    "R_first_noun": {
      "id": "R_first_noun",
      "type": "result",
      "coverage": "first.noun",
      "text": "عرفت مفتاح الجملة: الكلمة الأولى اسم. إذا كانت البداية جملة اسمية فابدأ بفحص المبتدأ والخبر، ولا تحكم بأن كل اسم في أول الكلام مبتدأ قبل النظر إلى تركيب الجملة."
    },
    "R_first_past": {
      "id": "R_first_past",
      "type": "result",
      "coverage": "first.verb.past",
      "text": "عرفت مفتاح الجملة: الكلمة الأولى فعل ماضٍ. راجع باب الفعل الماضي لاستكمال تعلّم أحكامه وعلامات بنائه."
    },
    "R_first_present": {
      "id": "R_first_present",
      "type": "result",
      "coverage": "first.verb.present",
      "text": "عرفت مفتاح الجملة: الكلمة الأولى فعل مضارع. راجع باب الفعل المضارع لاستكمال تعلّم رفعه ونصبه وجزمه وبنائه."
    },
    "R_first_imperative": {
      "id": "R_first_imperative",
      "type": "result",
      "coverage": "first.verb.imperative",
      "text": "عرفت مفتاح الجملة: الكلمة الأولى فعل أمر. راجع باب فعل الأمر لاستكمال تعلّم أحكامه وعلامات بنائه."
    },
    "R_first_particle_verb": {
      "id": "R_first_particle_verb",
      "type": "result",
      "coverage": "first.particle.verb",
      "text": "عرفت مفتاح الجملة: بدأت بحرف وبعده فعل. حدّد زمن الفعل وأثر الحرف."
    },
    "R_first_particle_noun": {
      "id": "R_first_particle_noun",
      "type": "result",
      "coverage": "first.particle.noun",
      "text": "عرفت مفتاح الجملة: بدأت بحرف وبعده اسم. حدّد نوع الحرف وأثره."
    }
  }
};
