...
const VISUAL_PATH_TOPIC_CODE_SET = new Set(VISUAL_PATH_TOPIC_CODES);

export function hasVisualPath(code) {
...
export const TOPICS = [
  {
    ...
    quizCount: Math.min(8, firstWordQuizExamples.length),
    level: 2,
    isReady: true,
  },
  {
...
export function resolveVisualPathTopic(requestedCode) {
  const code = requestedCode || "nominal-advanced";
  const topic = getTopicByCode(code);

  if (!topic) {
    return { status: "not-found", code, topic: null };
  }

  if (!hasVisualPath(code)) {
    return { status: "unavailable", code, topic };
  }
