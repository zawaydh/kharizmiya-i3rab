        ...
        if (id === "fael_form") {
            if (roleKind === "masdar") {
                ...
                if (pickedText.includes("كلمة"))
                    return `(${targetText}) ليست كلمة مفردة؛ إنها تركيب يمكن تأويله بمصدر صريح: (${taweel}). لذلك نختار «تركيب في تأويل اسم»، ثم نعربه مصدرًا مؤولًا في محل رفع فاعل.`;
            ...
            if (pickedText.includes("تركيب"))
                return `(${targetText}) ليست تركيبًا من حرف مصدري وفعل يؤول بمصدر؛ هي كلمة واحدة في هذا المستوى من التحليل. بعد ذلك نحدد: أهي اسم معرب أم اسم مبني؟`;
            return `انظر إلى المحدد كله: هل هو كلمة واحدة، أم تركيب يمكن تأويله باسم صريح مثل «أن تنجح» = «نجاحك»؟`;
        }
        if (id === "fael_masdar_term") {
            return `ثبت أن (${targetText}) تركيب في تأويل اسم؛ لأنه يمكن تحويله إلى مصدر صريح يؤدي معناه. المصطلح النحوي لهذا التركيب هو «مصدر مؤول»، وهو هنا في محل رفع فاعل.`;
        }
        if (id === "fael_word_inflection") {
            if (pickedText.includes("معرب") && roleKind !== "visible") {
                if (roleKind === "connected")
                    return `(${targetText}) ضمير متصل، والضمائر من الأسماء المبنية؛ لذلك لا نبحث عن ضمة تظهر على الضمير، بل نحدد نوعه من المبنيات ثم نقول: في محل رفع فاعل.`;
                return `(${targetText}) اسم مبني يلزم صورة واحدة، فلا تتغير حركة آخره بتغير الموقع؛ لذلك يكون هنا في محل رفع فاعل.`;
            }
            if (pickedText.includes("مبني") && roleKind === "visible")
                return `(${targetText}) اسم ظاهر معرب تتغير علامته بحسب موقعه، ولذلك نكمل إلى صورته وعلامة رفعه.`;
            if (roleKind === "connected")
                return `(${targetText}) ضمير متصل، والضمائر من الأسماء المبنية. بعد أن ثبت أنه مبني نحدد في الخطوة التالية نوعه: ضمير متصل.`;
            if (roleKind === "mabni")
                return `(${targetText}) اسم مبني يلزم صورة واحدة. بعد أن ثبت البناء نحدد نوع المبني نفسه من المثال.`;
            return `(${targetText}) اسم ظاهر معرب؛ لذلك تظهر عليه علامة رفع أو تكون مقدرة، وننتقل إلى تحديد صورته.`;
        }
