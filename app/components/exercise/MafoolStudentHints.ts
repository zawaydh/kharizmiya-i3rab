    ...
    if (id.startsWith("mafool_")) {
        ...
        if (id === "mafool_word_inflection") {
            if (pickedText.includes("معرب") && roleKind !== "visible") {
                if (roleKind === "connected")
                    return `(${targetText}) ضمير متصل، والضمائر من الأسماء المبنية؛ لذلك لا نبحث عن فتحة تظهر عليه، بل نحدد نوعه من المبنيات ثم نقول: في محل نصب مفعول به.`;
                return `(${targetText}) اسم مبني يلزم صورة واحدة، فلا تتغير حركة آخره بتغير الموقع؛ لذلك يكون هنا في محل نصب مفعول به.`;
            }
            if (pickedText.includes("مبني") && roleKind === "visible")
                return `(${targetText}) اسم ظاهر معرب تتغير علامته بحسب موقعه، ولذلك نكمل إلى صورته وعلامة نصبه.`;
            if (roleKind === "connected")
                return `(${targetText}) ضمير متصل، والضمائر من الأسماء المبنية. بعد أن ثبت البناء نحدد في الخطوة التالية نوعه: ضمير متصل.`;
            if (roleKind === "mabni")
                return `(${targetText}) اسم مبني يلزم صورة واحدة. بعد أن ثبت البناء نحدد نوع المبني نفسه من المثال.`;
            return `(${targetText}) اسم ظاهر معرب؛ لذلك تظهر عليه علامة نصب أو ما ينوب عنها، وننتقل إلى تحديد صورته.`;
        }
