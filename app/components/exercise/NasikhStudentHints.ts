...
export function nasikhStudentHintText(node: PedagogyNode | null | undefined, picked?: ExerciseAnswer, state?: PedagogyState): string | undefined {
  ...
  const target = state?.currentTarget || "الفعل";
    if (id.includes("kana")) {
        ...
        const kanaMatch = sentence.match(/(?:ما\s+)?(?:كان|أصبح|أمسى|أضحى|ظل|بات|صار|ليس|زال|برح|فتئ|انفك)(?:َ|ُ|ِ|ْ|ّ|ً|ٌ|ٍ|\s)*/u);
        const kanaLabel = String(kanaMatch?.[0] || "الفعل الناسخ").trim();
        const afterKana = `بعد الفعل الناسخ (${kanaLabel})`;
        ...
        if (id === "kana_target") {
            ...
            if (pickedText.includes("اسم") && String(state?.facts?.targetRole) === "khabar") return `${afterKana} ثبت أن (${currentTarget}) أتمت المعنى عن اسم الناسخ؛ لذلك هي خبره، لا اسمه.`;
            if (pickedText.includes("خبر") && String(state?.facts?.targetRole) === "ism") return `${afterKana} اسأل: من صاحب المعنى الذي أسند إليه الخبر؟ في هذا المثال هو (${currentTarget})، لذلك هو اسم الناسخ.`;
            if (pickedText.includes("غير ظاهر") && String(state?.facts?.targetRole) !== "hidden_ism") return `${afterKana} لا نقدّر اسمًا مستترًا ما دام بعد الناسخ اسم صريح يصلح أن يكون اسمه. افحص الجملة حتى نهايتها.`;
        }
        ...
        if (id === "kana_masdar_site") {
            ...
            return role === "ism"
              ? `${afterKana} المصدر المؤول هو صاحب المعنى الذي أُسند إليه الخبر؛ لذلك هو في محل رفع اسم الناسخ.`
              : `${afterKana} المصدر المؤول أتم المعنى عن اسم الناسخ؛ لذلك هو في محل نصب خبر الناسخ.`;
        }
        ...
        if (id === "kana_ism_number" || id === "kana_khabar_single_number") {
            ...
            if (pickedText.includes("الأسماء الخمسة")) return `الأسماء الخمسة هي: أب، أخ، حم، فو، ذو بمعنى صاحب. وتشترط لإعرابها بالحروف أن تكون مفردة، مكبرة، مضافة، وغير مضافة إلى ياء المتكلم. هل (${currentTarget}) واحد منها؟`;
        ...
        if (id === "kana_ism_ending") {
            ...
            if (pickedText.includes("صحيح الآخر") && actualEnding !== "sahih") {
                if (actualEnding === "attached_ya") return `اخترتَ «صحيح الآخر»، لكن (${currentTarget}) متصل بياء المتكلم: افصل الياء أولًا؛ فهي ضمير مضاف إليه، ثم انظر إلى أصل الاسم.`;
                return `اخترتَ «صحيح الآخر»، لكن آخر (${currentTarget}) الأصلي حرف علة. وحروف العلة هي: الألف، والواو، والياء؛ لذلك لا نعالجه معاملة الاسم الصحيح الآخر.`;
            }
            if (pickedText.includes("متصل بياء المتكلم") && actualEnding !== "attached_ya") return `اخترتَ «متصل بياء المتكلم»، لكن الياء في هذا المثال ليست ضمير المتكلم المضاف إليه. افحص آخر (${currentTarget}) الأصلي قبل الحكم.`;
            if (pickedText.includes("معتل الآخر") && actualEnding !== "moatal") {
                if (actualEnding === "attached_ya") return `اخترتَ «معتل الآخر»، لكن الياء في (${currentTarget}) ياء المتكلم وليست من أصل الاسم. افصلها أولًا، ثم افحص آخر الأصل.`;
    ...
    if (id.includes("inna")) {
        ...
        if (!picked) {
            ...
            if (id === "inna_khabar_single_inflection") {
                if (pickedText.includes("معرب") && nounKind === "mabni") return `اخترتَ «اسم معرب»، لكن (${currentTarget}) اسم مبني يلزم صورة واحدة؛ لذلك لا تظهر عليه علامة رفع، بل يكون في محل رفع خبر ${innaParticleName(state)}.`;
                if (pickedText.includes("مبني") && nounKind === "mu3rab") return `اخترتَ «اسم مبني»، لكن (${currentTarget}) اسم معرب تتغير علامته بحسب موقعه؛ لذلك نكمل إلى صورة الاسم وعلامة الرفع.`;
                if (nounKind === "mabni") return `(${currentTarget}) اسم مبني يلزم صورة واحدة، لذلك لا نبحث عن ضمة ظاهرة، بل نحدد نوعه ثم نقول: في محل رفع خبر ${innaParticleName(state)}.`;
                return `(${currentTarget}) اسم معرب تتغير علامته بحسب موقعه، لذلك نكمل إلى صورته وعلامة رفعه.`;
            }
        if (id === "inna_ism_built" || id === "inna_khabar_single_built") {
                if (nounKind === "connected_damir") return `(${currentTarget}) ضمير متصل، والضمائر من الأسماء المبنية؛ لذلك نوع الاسم المبني هنا: ضمير متصل.`;
            ...
            if (id === "inna_ism_number" || id === "inna_khabar_single_number") {
                if (number === "five") return `افحص هل (${currentTarget}) من الأسماء الخمسة: أب، أخ، حم، فو، ذو بمعنى صاحب. إذا كان مفردًا، مكبرًا، مضافًا، ومضافًا إلى غير ياء المتكلم، أعرب بالحروف.`;
        ...
        if (id === "inna_ism_number" || id === "inna_khabar_single_number") {
            ...
            if (pickedText.includes("الأسماء الخمسة")) return `الأسماء الخمسة هي: أب، أخ، حم، فو، ذو بمعنى صاحب. وتعرب بالحروف إذا كانت مفردة، مكبرة، مضافة، ومضافة إلى غير ياء المتكلم. هل (${currentTarget}) واحد منها واستوفى هذه الشروط؟`;
