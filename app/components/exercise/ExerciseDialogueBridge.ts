...
export function bridgeKickerText(tree: PedagogyTree, node: PedagogyNode | null | undefined, state: PedagogyState, title?: string, completedPieces: string[] = []) {
    ...
    if (!last) {
        ...
        if (start === "tawabi_naat_discovery")
            return `مسار النعت: نثبت أولًا أن (${target}) وصفٌ لاسم قبله، ثم ننقل حالة المنعوت ونختار العلامة.`;
        if (start === "tawabi_atf_discovery")
            return `مسار العطف: نبحث أولًا عن حرف العطف الذي أشرك (${target}) مع ما قبله، ثم ننقل الحالة ونختار العلامة.`;
        if (start === "tawabi_tawkid_discovery")
            return `مسار التوكيد: نثبت أولًا أن (${target}) قوّى معنى ما قبله، ثم نحدد نوع التوكيد والحالة والعلامة.`;
        if (start === "tawabi_badal_discovery")
            return `مسار البدل: نثبت أولًا أن (${target}) بيّن المقصود من الاسم السابق أو جزءًا أو معنى منه، ثم ننقل الحالة ونختار العلامة.`;
