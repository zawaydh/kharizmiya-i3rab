...
export function extendedTopicOpeningDialogueLine(
  ...
  if (id === "munada_kind") {
    return `ثبت أن (${target}) منادى. الآن نوع المنادى هو الذي يحدد هل يُبنى في محل نصب أم يُعرب منصوبًا؛ فما نوعه في هذه الجملة؟`;
  }
  if (id === "munada_shape") {
  ...
  if (id === "naib_form") {
    return `عرفنا أن (${target}) نائب فاعل، ونائب الفاعل مرفوع. الآن نحدد صورته: أهو اسم ظاهر معرب، أم اسم مبني، أم ضمير متصل؟`;
  }
  if (id === "naib_shape") {
