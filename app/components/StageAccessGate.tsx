...
type StageRequirement = "learn" | "practice" | "quiz" | "certificate" | "texts";
type GateStatus = "loading" | "auth" | "allowed" | "blocked";
...
const DEFAULT_TEXT: Partial<Record<StageRequirement, string>> = {
...
export default function StageAccessGate({
  topicCode,
  level = 2,
  require = "learn",
  children,
  ...
  return (
    <section className="student-flow-gate card card-glow" dir="rtl">
      ...
      <div className="student-flow-gate-actions">
        {require === "practice" ? (
          <a className="btn btn-primary" href={nextLearn}>اذهب إلى التعلّم الموجّه</a>
        ) : null}
        {require === "quiz" || require === "certificate" ? (
          <a className="btn btn-primary" href={nextPractice}>اذهب إلى التدريب</a>
        ) : null}
        {require === "texts" ? (
          <a className="btn btn-primary" href={nextQuiz}>اذهب إلى الاختبار النهائي</a>
        ) : null}
